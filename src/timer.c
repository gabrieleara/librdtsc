...
// NOTICE: THIS FILE IS LINUX ONLY!!!!
...
// #include <rdtsc_debug.h>
// #include <rdtsc_eal.h>
// #include <rdtsc_lcore.h>
// #include <rdtsc_log.h>
// #include <rdtsc_memory.h>

// #include "eal_internal_cfg.h"
// #include "eal_private.h"

#ifdef LIBRDTSC_USE_HPET
enum timer_source timer_source = RDTSC_TIMER_HPET;
#else
enum timer_source timer_source = RDTSC_TIMER_TSC;
#endif
 ...
 */
static void *hpet_msb_inc(void *arg) {
    SET_USED(arg);
    uint32_t t;
...
uint64_t rdtsc_get_hpet_hz(void) {
    ...
    //     rdtsc_panic("Error, HPET called, but no HPET present\n");
 ...
 */
int rdtsc_eal_hpet_init(int make_default) {
    int fd, ret;
    // struct internal_config *internal_conf = eal_get_internal_configuration();

    // if (internal_conf->no_hpet) {
    //     RDTSC_LOG(NOTICE, EAL, "HPET is disabled\n");
    //     return -1;
    // }

    fd = open(DEV_HPET, O_RDONLY);
    if (fd < 0) {
        // RDTSC_LOG(ERR, EAL, "ERROR: Cannot open " DEV_HPET ": %s!\n",
        //         strerror(errno));
    ...
    if (eal_hpet == MAP_FAILED) {
        ...
        //         "To run without using HPET, unset LIBRDTSC_USE_HPET "
    ...
    // RDTSC_LOG(INFO, EAL, "HPET frequency is ~%" PRIu64 " kHz\n",
    //         eal_hpet_resolution_hz / 1000);

    eal_hpet_msb = (eal_hpet->counter_l >> 30);

    // TODO: this next part of the msb is tricky, maybe HPET is
    // not a good idea after all...

    // /* create a thread that will increment a global variable for
    //  * msb (hpet is 32 bits by default under linux) */
    // ret = rdtsc_ctrl_thread_create(&msb_inc_thread_id, "hpet-msb-inc", NULL,
    //                              hpet_msb_inc, NULL);
    // if (ret != 0) {
    //     RDTSC_LOG(ERR, EAL, "ERROR: Cannot create HPET timer thread!\n");
    //     internal_conf->no_hpet = 1;
    //     return -1;
    // }

    if (make_default)
        timer_source = RDTSC_TIMER_HPET;
    return 0;
}
#endif

uint64_t rdtsc_get_tsc_freq(void) {
